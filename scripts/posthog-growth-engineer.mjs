...
function parseArgs(argv) {
    const args = {
        ...
        out: 'data/posthog-growth-engineer/issues.generated.json',
        maxIssues: 5,
        createIssues: false,
...
Usage:
  node scripts/posthog-growth-engineer.mjs --analytics <file> [options]

Required:
  --analytics <file>     PostHog summary JSON
...
function buildNoSignalsError(sourceEntries) {
    ...
    return [
        ...
        'Next steps:',
        '- Pin PostHog MCP to the intended project for scheduled runs, then rerun bounded MCP queries.',
        '- Verify the PostHog project has release/app version events in the requested window, or enable additional sources such as ASC CLI, RevenueCat, Sentry, or feedback.',
    ].join('\n');
}
...
function pathScoreBias(relativePath) {
    ...
    if (relativePath === 'scripts/posthog-growth-engineer.mjs') {
        score -= 12;
    }
    return score;
}
...
async function githubApiRequest(url, token, options = {}) {
    const response = await fetch(url, {
        ...
        headers: {
            ...
            'User-Agent': 'posthog-growth-engineer-mvp',
            ...(options.headers || {}),
        },
    });
    return response;
